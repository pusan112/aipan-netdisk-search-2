...
const handleSwitchVideoStatus = () => {
    if (player && videoPlayStatus.value) {  // 只在TV开启时可用
        ...
        <div ref="tvContainer" class="relative w-full max-w-6xl mx-auto px-8 tv-container"
            ...
            <div class="tv-frame rounded-[2.5rem] shadow-2xl p-8 
                ...
                <div class="tv-control-panel mt-6 px-4">
                    ...
                    <div class="tv-buttons-panel flex flex-col md:flex-row items-center justify-between gap-4 md:gap-6 bg-gradient-to-b from-neutral-800/50 to-neutral-900/50 p-3 md:p-4 rounded-xl backdrop-blur-sm border border-neutral-700/30">
                        <div class="tv-left-controls flex flex-wrap justify-center md:justify-start gap-2 md:gap-3 w-full md:w-auto">
                            ...
                            <button class="control-btn" @click="handleMute">
                                <span class="flex items-center gap-2">
                                    <i :class="!videoMuted ? 'fas fa-volume-up' : 'fas fa-volume-mute'"></i>
                                    <span class="hidden sm:inline">{{ !videoMuted ? '静音' : '取消静音' }}</span>
                                </span>
                            </button>
                            <button class="control-btn" @click="handleSwitchVideoStatus">
                                <span class="flex items-center gap-2">
                                    <i :class="!isPlaying ? 'fas fa-play' : 'fas fa-pause'"></i>
                                    <span class="hidden sm:inline">{{ !isPlaying ? '播放' : '暂停' }}</span>
                                </span>
                            </button>
                        </div>
                        ...
                        <div class="tv-decoration flex items-center justify-center my-2 md:my-0">
                            <button class="control-power-btn" 
                                ...
                                <div class="power-status">
                                    <div class="status-dot" :class="{'active': videoPlayStatus}"></div>
                                </div>
                            </button>
                        </div>
            ...
            <div class="tv-channel-dialog w-[90%] max-w-6xl h-[90vh] bg-white/5 rounded-2xl 
                ...
                <div class="tv-channel-content">
                    <div class="channel-search">
                        <input class="tv-input" type="text" v-model="videoSrc" placeholder="请输入视频链接">
                        <button class="tv-btn primary p-2 text-sm rounded-md" @click="handleSwitchSource(videoSrc)">切换视频</button>
                    </div>
                        ...
                        <div class="space-y-3" v-if="channelCategory === 3">
                                ...
                                <div class="alist-item setting-item"
                                    ...
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <div class="flex-1">
                                            ...
                                            <div class="text-sm text-white/60 truncate">{{ item.link }}</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="space-y-3" v-else>
                                <div class="alist-item"
                                    ...
                                    <div class="flex items-center space-x-3">
                                        <!-- Folder Icon -->
                                        <svg v-if="item.is_dir" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
